...
def handle_save_contact_query(bot, call):
    ...
    if call.data == "save_contact_yes" and user_sessions[chat_id]["action"] == "process_contact":
        contact = user_sessions[chat_id]["contact"]
        names = contact["full_name"].split(' ', 1)
        first_name = names[0]
        last_name = names[1] if len(names) > 1 else ""
        add_contact(chat_id, contact["telephone"], first_name, last_name)
        # add_contact(chat_id, first_name, last_name, contact["telephone"])
        bot.edit_message_text(f"Contact {contact['full_name']} has been saved successfully!", chat_id=chat_id, message_id=call.message.message_id)
    elif call.data == "save_contact_no":
        bot.edit_message_text("Contact saving cancelled.", chat_id=chat_id, message_id=call.message.message_id)
