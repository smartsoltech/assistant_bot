...
def start_handle(message):
    ...
    if chat_id not in user_sessions:
        user_sessions[chat_id] = {"action": "", "registration_open": False}
    
    # Проверка статуса регистрации
    if user_sessions[chat_id]["registration_open"]:
        # Извлечение данных пользователя
        user_id = message["from_user"]["id"]
        first_name = message["from_user"]["first_name"]
        last_name = message["from_user"].get("last_name", "")
        username = message["from_user"]["username"]
        chat_id = message["from_user"]["id"]
        # Сохранение пользователя в БД
        # db_operations.add_family_member(first_name, last_name, user_id, chat_id)

        # Закрыть регистрацию после успешного добавления
        user_sessions[chat_id]["registration_open"] = False
...
def add_member_link(message):
    ...
    qrname=f"./tmp_files/{unique_code}.png"
    img.save(qrname)
    ...
    # Открываем регистрацию для пользователя, который перешел по ссылке
    user_sessions[chat_id]["registration_open"] = True

    # Здесь можете отправить пользователю сообщение с инструкциями о том, что он может нажать кнопку "start" для завершения регистрации
    bot.send_message(chat_id, f'Введите комментарий для этого пользователя!')
...
def handle_text(message):
    ...
    if chat_id in user_sessions:
        ...
        # добавьте другие проверки действий здесь по мере необходимости
            ...
            # Если неизвестное действие, обрабатываем как обычный текст
            text_handle(bot, message)
    else:
        text_handle(bot, message)  # Обработка сообщений без конкретной сессии

@bot.message_handler(func=lambda m: user_sessions.get(m.chat.id, {}).get("action") == "addmember")
def save_new_member(message):
    # Здесь вы можете получить user_code и другие необходимые данные из user_sessions или других источников
    user_code = user_sessions.get(message.chat.id, {}).get("code")
    first_name = message.text.split(' ', 1)[0]
    last_name = message.text.split(' ', 1)[1] if ' ' in message.text else ""

    # Здесь мы сохраняем нового члена семьи в базу данных
    add_family_member(first_name, last_name, user_code, message.chat.id)
    bot.send_message(message.chat.id, f"New member {first_name} {last_name} has been added.")
