...
def handle_calendar_callback(bot, call):
    try:
        ...
        if not result and key:
            ...
            action = user_sessions.get(chat_id, {}).get("action")
            if action:
                ...
                logger.warning(f"No action found in user_sessions for chat_id: {chat_id}")
    except ApiTelegramException as e:
        logger.error(f"Telegram API Error in handle_calendar_callback: {e}")
    except Exception as e:
        logger.error(f"Error in handle_calendar_callback: {e}")
