...
def add_new_event(message):
    ...
    date = current_shown_dates.get(chat_id)
    if date:
        bot.send_message(chat_id=chat_id, text="Please select a date", reply_markup=telegramcalendar.create_calendar())
    else:
        now = datetime.datetime.now()
        bot.send_message(chat_id=chat_id, text="Please select a date", reply_markup=telegramcalendar.create_calendar(now.year, now.month))
...
def add_new_event(message):
    ...
    date = current_shown_dates.get(chat_id)
    if date:
        bot.send_message(chat_id=chat_id, text="Please select a date", reply_markup=DetailedTelegramCalendar.create_calendar())
    else:
        now = datetime.datetime.now()
        bot.send_message(chat_id=chat_id, text="Please select a date", reply_markup=DetailedTelegramCalendar.create_calendar(now.year, now.month))
...
def handle_callback_query(call):
    ...
    if command == 'add_event':
        bot.send_message(call.message.chat.id, "Пожалуйста, отправьте дату и описание события в формате YYYY-MM-DD Описание", reply_markup=DetailedTelegramCalendar.create_calendar())
        user_sessions[call.message.chat.id] = ('waiting_for_event_data', member_id)
    elif command == 'add_reminder':
        bot.send_message(call.message.chat.id, "Пожалуйста, отправьте дату и описание напоминания в формате YYYY-MM-DD Описание", reply_markup=DetailedTelegramCalendar.create_calendar())
        user_sessions[call.message.chat.id] = ('waiting_for_reminder_data', member_id)
@bot.callback_query_handler(func=lambda call: call.data.startswith(DetailedTelegramCalendar.ID))

def handle_query(call):
    selected, date = DetailedTelegramCalendar().process_selection(call)
    if selected:
...
def handle_query(call):
    ...
    if selected:
        ...
        bot.edit_message_text("Please select a date: ",
