...
import telebot
from db_operations import save_contact, add_family_member, add_event, add_reminder, get_family_members, database
...
from database_functions import add_reminder_to_db
from datetime import datetime
...
def handle_date_description_input(message):
    ...
    if user_sessions[user_id] == 'add_reminder':
        add_reminder_to_db(date_obj, description, user_id)
        bot.send_message(message.chat.id, "Напоминание успешно добавлено!")
        del user_sessions[user_id]  # очистите временные данные после завершения операции

def add_reminder_to_db(date, description, user_id):
    session = Session()
    new_reminder = Reminder(
        date=date,
        description=description,
        family_member_id=user_id  # Примечание: здесь предполагается, что ID члена семьи совпадает с user_id в Telegram. Возможно, вам потребуется модифицировать эту часть.
    )
