...
def handle_full_reset_callback(call):
    if call.data == "full_reset_confirm":
       if full_reset:
           bot.edit_message_text("Таблицы были успешно сброшены!", call.message.chat.id, call.message.message_id)
        else bot.edit_message_text("Таблицы не были успешно сброшены!", call.message.chat.id, call.message.message_id)
           else:
